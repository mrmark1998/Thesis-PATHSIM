function Optimal_path = A_Star5(map, startp, endp)
 ...
 plot(startp(2), startp(1), 'ro', 'MarkerSize', 10);
 plot(endp(2), endp(1), 'go', 'MarkerSize', 10);
 p=plot(Optimal_path(j,2)+.5,Optimal_path(j,1)+.5,'y*', 'MarkerSize', 12);
 
 j=j-1;
 for i=j:-1:1
  pause(.05);
  set(p,'YData',Optimal_path(i,1)+.5,'XData',Optimal_path(i,2)+.5);
 drawnow ;
 end;
 plot(Optimal_path(:,2)+.5,Optimal_path(:,1)+.5, 'm', ... 
               'LineStyle', ':', ...
               'LineWidth',3);
else
 pause(1);
